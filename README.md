...
`qa:stealth-systems` 是稳定发布门禁，会分别生成门楔、拐角镜和临时断电三个正式场景的浏览器截图与结构化报告，并要求高画质锁、正式 GLB 来源和零诊断。`qa:stealth-systems:extended` 额外执行足迹、导演、多主题/多视口与热资源耐久检查，作为非阻断扩展门禁。未绑定最终 Git SHA 的本地报告只属于候选证据，不代表已经部署。
