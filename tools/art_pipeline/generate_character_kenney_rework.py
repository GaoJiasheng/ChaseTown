"""Build Task B/C/D character FBX packages from CC0 Kenney humanoid bases.

Run with:
  blender --background --python tools/art_pipeline/generate_character_kenney_rework.py

This script intentionally does not use the previous MB-Lab/procedural body
route. The visible humanoid base meshes come from downloaded Kenney CC0 packs,
then Blender adds role-specific clothing/accessory geometry, shared Humanoid
bone names, PBR texture assignments, preview renders, wireframes, and reports.
...
KENNEY = ROOT / "art-source" / "_Source" / "Characters" / "Kenney"
...
MINI_FBX = KENNEY / "mini" / "Models" / "FBX format"
ANIMATED_FBX = KENNEY / "animated_protagonists" / "Model"
...
SPECS = [
    {
        ...
        "budget": (18000, 30000),
        "texture_res": "2K",
        "source": ANIMATED_FBX / "characterMedium.fbx",
        "source_pack": "Kenney Animated Characters: Protagonists",
    ...
    },
    {
        ...
        "budget": (12000, 20000),
        "texture_res": "2K",
        "source": MINI_FBX / "character-female-c.fbx",
        "source_pack": "Kenney Mini Characters",
        "source_url": "https://kenney.nl/assets/mini-characters",
        "subdivision": 2,
    },
    {
        "role": "Villain",
    ...
    },
    {
        ...
        "budget": (14000, 22000),
        "texture_res": "2K",
        "source": MINI_FBX / "character-male-f.fbx",
        "source_pack": "Kenney Mini Characters",
        "source_url": "https://kenney.nl/assets/mini-characters",
        "subdivision": 2,
    },
    ...
    },
    {
        ...
        "budget": (12000, 18000),
        "texture_res": "2K",
        "source": MINI_FBX / "character-male-b.fbx",
        "source_pack": "Kenney Mini Characters",
        "source_url": "https://kenney.nl/assets/mini-characters",
        "subdivision": 2,
    },
...
def finish_mesh(
    ...
    if subdivision:
        mod = obj.modifiers.new("Kenney_Surface_Rebuild", "SUBSURF")
...
def materialize_source_meshes(meshes: list[object], main_mat, accent_mat, role: str, style: str, subdivision: int) -> list[object]:
    ...
    for obj in meshes:
        ...
        if "head" in name:
            mat = accent_mat
            uv = (0.0, 0.5, 0.5, 1.0)
        elif "charactermedium" in name:
            mat = accent_mat
            uv = (0.0, 0.5, 0.5, 1.0)
        else:
            mat = main_mat
            uv = (0.0, 0.0, 1.0, 1.0)
        finish_mesh(obj, mat, uv, bevel=None, subdivision=subdivision)
        objects.append(obj)
    return objects
...
def add_role_features(role: str, style: str, height: float, base_meshes: list[object], main_mat, accent_mat) -> list[object]:
    ...
    animated_base = any("charactermedium" in obj.name.lower() for obj in base_meshes)
    objects: list[object] = []

    if photoreal or animated_base:
        src = base_meshes[0]
        ...
        if role == "Kid":
            ...
            for item in (
                ...
                subset_mesh(src, "Boot_Overlay", lambda c: c.z < h * 0.105, accent_mat, (0.5, 0.0, 1.0, 0.5), h * 0.008, 0),
            ):
                if item:
                    objects.append(item)
        else:
            for item in (
                ...
                subset_mesh(src, "Uniform_Shoe_Overlay", lambda c: c.z < h * 0.090, accent_mat, (0.5, 0.0, 1.0, 0.5), h * 0.007, 0),
            ):
                if item:
                    objects.append(item)
    ...
    if role == "Kid":
        ...
        objects += [
            add_uv_sphere("Deep_Hood", (0, -h * 0.020, h * 0.890), (h * 0.112, h * 0.080, h * 0.072), main_mat, (0, 0, 1, 1), segments=52, rings=22, subdivision=1 if photoreal else 0),
            add_uv_sphere("Face_Shadow", (0, -h * 0.094, h * 0.855), (h * 0.056, h * 0.012, h * 0.046), accent_mat, (0.5, 0.0, 1.0, 0.5), segments=30, rings=14),
    ...
    if not blind:
        eye_z = h * 0.880
        objects += [
            add_uv_sphere("Eye_L", (-h * 0.031, -h * 0.146, eye_z), (h * 0.008, h * 0.004, h * 0.007), accent_mat, (0.5, 0.0, 1.0, 0.5), segments=18, rings=10),
            add_uv_sphere("Eye_R", (h * 0.031, -h * 0.146, eye_z), (h * 0.008, h * 0.004, h * 0.007), accent_mat, (0.5, 0.0, 1.0, 0.5), segments=18, rings=10),
            add_uv_sphere("Nose_SoftBridge", (0, -h * 0.150, h * 0.850), (h * 0.006, h * 0.006, h * 0.013), accent_mat, (0.0, 0.0, 0.5, 0.5), segments=16, rings=8),
            add_tube("Mouth_Line", [(-h * 0.017, -h * 0.152, h * 0.820), (h * 0.017, -h * 0.152, h * 0.820)], h * 0.0015, accent_mat, (0.5, 0.0, 1.0, 0.5)),
        ]
    else:
        eye_y = -h * 0.360
        eye_z = h * 0.865
...
def generate(spec: dict) -> dict:
    ...
    report = {
        ...
        "source": {
            "route": "Kenney CC0 humanoid base mesh plus Blender role rework",
            "pack": spec["source_pack"],
            "source_file": str(spec["source"].relative_to(ROOT)),
            "source_url": spec["source_url"],
            "license_file": str((KENNEY / ("mini" if "Mini" in spec["source_pack"] else "animated_protagonists") / "License.txt").relative_to(ROOT)),
        ...
        "geometry_review": {
            ...
            "surface_detail_layers": ["CC0 humanoid source mesh", "PBR image texture", "stitch/wear map layer", "beveled accessories", "role silhouette clothing"],
        },
        "quality_gate": {
...
def main() -> int:
    ...
    summary = {
        ...
        "unity_validation": "blocked_no_active_unity_license",
        "route": "Kenney CC0 humanoid base mesh rework",
        "results": results,
    }
